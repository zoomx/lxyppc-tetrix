...
chrt_grid_prep:
        case GRID_PREP:
        	...
        	*/
        	...
        	xStart += (GetTextHeight(pCh->prm.pAxisLabelsFont)+
        			   (GetTextWidth(temp2Xchar, pCh->prm.pGridLabelsFont)>>1));  
        ...
        case VALUE_LABEL_DRAW_SET:
			...
			if (GetState(pCh, CH_PERCENT)) {
       		...
       		if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) { 
				...
				MoveTo(	xStart-GetTextWidth((&tempStr[STR_CHAR_CNT-j]), pCh->prm.pGridLabelsFont), 
...
chrt_bar_data_draw_value:
		case BAR_DATA_DRAW_VALUE:
			...
			if (GetState(pCh, CH_3D_ENABLE)) {
				if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) { 
					MoveTo(	xStart+dTemp+barDepth+3,
							y-(temp*varCtr)-((temp+GetTextHeight(pCh->hdr.pGolScheme->pFont))>>1)-barDepth); 
				} else {
					MoveTo(	x+barDepth+(temp*varCtr)+(temp>>1)-(GetTextWidth(&tempStr[STR_CHAR_CNT-j], pCh->hdr.pGolScheme->pFont)>>1),
				...
				if (GetState(pCh, CH_BAR_HOR) == CH_BAR_HOR) { 
					...
					MoveTo(	x+((temp>>1)-(GetTextWidth(&tempStr[STR_CHAR_CNT-j], pCh->hdr.pGolScheme->pFont)>>1))+(temp*varCtr), 
        ...
        case PIE_PREP:
			...
			*/
			...
			if (((pCh->hdr.right-pCh->hdr.left)-temp-((x+y)<<1)) < 				\
				((pCh->hdr.bottom-pCh->hdr.top-GetTextHeight(pCh->prm.pTitleFont))-temp-(GetTextHeight(pVarFont)<<1))) 
			{
				...
				if (x+y)
					z = ctry - (pCh->hdr.top+(CH_MARGIN<<1)+GetTextHeight(pCh->prm.pTitleFont)+(GetTextHeight(pVarFont)<<1));
				else	
        ...
        case PIE_DRAW_SECTOR_LOOP_CREATE_STRINGS:
			...
			if (GetState(pCh, CH_VALUE) || GetState(pCh, CH_PERCENT)) {
				...
				if ((m+(dTemp>>1) >= 0) && (m+(dTemp>>1) <= 90)) {
					...
					if ((dTemp < GetTextHeight(pVarFont)) ||
						(pieLabelYPos > pieY)	) {
					...
					if ((dTemp < GetTextHeight(pVarFont)) ||
						...
						if ((m+(dTemp>>1)) < 180) {
							...
							if ((abs(pieY-pieLabelYPos2)/abs(pieX-pieLabelXPos)) >= 1) {
								...
								if ((SHORT)(pieLabelXPos-GetTextWidth(&tempStr[STR_CHAR_CNT-h], pVarFont)) <= (pCh->hdr.left+CH_MARGIN)) {
									pieLabelXPos = (pCh->hdr.left+CH_MARGIN+GetTextWidth(&tempStr[STR_CHAR_CNT-h], pVarFont));
								}	
							}		
						}	
						...
						MoveTo(pieX-GetTextWidth(&tempStr[STR_CHAR_CNT-h], pVarFont)-
									GetTextWidth(tempXchar, pVarFont), 
							   pieY); 
