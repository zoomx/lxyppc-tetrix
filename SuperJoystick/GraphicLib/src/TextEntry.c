		...
		case TE_START:
			...
			if(GetState(pTe,TE_HIDE))
			{
			    ...
			    if(GetState(pTe,TE_DRAW)) {
				    ...
				    GOLPanelDraw(pTe->hdr.left,pTe->hdr.top,pTe->hdr.right,pTe->hdr.bottom,0,
...
te_draw_panel:		
		...
		case TE_INIT_DRAW_EDITBOX:
			...
			GOLPanelDraw(	pTe->hdr.left,pTe->hdr.top,
							pTe->hdr.right,pTe->hdr.top+GetTextHeight(pTe->pDisplayFont)+(GOL_EMBOSS_SIZE<<1),
							0, pTe->hdr.pGolScheme->Color1;,
...
te_draw_key_set_panel_st:			
			if (CountOfKeys < (pTe->horizontalKeys*pTe->verticalKeys))	{
                ...
                // set up the panel
				GOLPanelDraw(	pKeyTemp->left,pKeyTemp->top,
								pKeyTemp->right,pKeyTemp->bottom,0,
								faceClr,
								embossLtClr,
								embossDkClr,
								NULL,
...
te_update_string_init_st:
			...
			SetClipRgn(	pTe->hdr.left+GOL_EMBOSS_SIZE, 
