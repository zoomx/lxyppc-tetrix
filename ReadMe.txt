Automatically exported from code.google.com/p/lxyppc-tetrix

Tetrix.sln         为VC2005项目文件
Tetrix_2003.sln    为VC2003项目文件
STM32文件夹下面为STM32固件代码，IAR4.42编译


Tetrix.sln         Solution file for VC2005
Tetrix_2003.sln    Solution file for VC2003
.\STM32            Source code for the STM32 firmware, compile by IAR4.42 for ARM

(Automatically translated from chinese)
With the camera to play Tetris
STM32 virtual with a camera device, and play Tetris effect in the camera head as follows: will be more into the computer with a USB camera device, open the camera you can see Tetris interface

Camera Tetris
Use the STM32 to simulate a USB camera And play the classic tetris game on it
