...
typedef struct ring_buffer_content_t ring_buffer_content_t;

struct ring_buffer_t
{
    uint32_t    len;        // ring buffer length
    uint32_t    size;       // ring buffer size
    ring_buffer_content_t*  content;
};
...
int ring_buf_push(const ring_buffer_t* buf,const void *p ,uint32_t len)
{
    ...
    ring_buffer_memcpy(&buf->content->buffer[(buf->content->wrIdx & (buf->len-1))*buf->size],p,len);
    buf->content->wrIdx++;
    return 1;
}
...
int ring_buf_pop(const ring_buffer_t* buf, void* p ,uint32_t len)
{
    ...
    ring_buffer_memcpy(p, &buf->content->buffer[(buf->content->rdIdx & (buf->len-1))*buf->size],len);
    buf->content->rdIdx++;
    return 1;
}
